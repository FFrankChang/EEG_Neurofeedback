...
for folder in os.listdir(base_dir):
    if subject in folder.lower() and ('easy' in folder.lower() or 'hard' in folder.lower()):
        ...
        if os.path.exists(file_path):
            ...
            # Analyze the first 5 deceleration periods
            stats = {}
            for idx, (start, end) in enumerate(deceleration_periods[:5], start=1):
                ...
                stats[f"Mean Change Rate {idx}"] = steering_angles.diff().mean()
                stats[f"Variance {idx}"] = steering_angles.var()
                stats[f"Coefficient of Variation {idx}"] = steering_angles.std() / steering_angles.mean()
