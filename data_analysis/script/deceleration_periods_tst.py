...
base_dir = r"D:\gitee\EEG_Neurofeedback\data\test"   # Adjust this path
...
for folder in os.listdir(base_dir):
    if 'lxk' in folder.lower() and ('easy' in folder.lower() or 'hard' in folder.lower()):
        ...
        if os.path.exists(file_path):
            ...
            stats = {key: 0 for i in range(1, 6) for key in (f"Mean Change Rate {i}", f"Variance {i}", f"Coefficient of Variation {i}")}
...
results_df.to_csv("results02.csv", index=False)
