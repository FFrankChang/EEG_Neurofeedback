...
class DataProcessor:
    def __init__(self, subject, file_paths):
        ...
        columns = ["subject", "experiment_no", "scenario", "condition", "Steering_Angle_Std"]
    ...
    def calculate_statistics(self, folder):
        ...
        mode_switched_time = self.carla_data[self.carla_data['Mode_Switched'] == 'Yes']['timestamp'].min()
        fifth_end_time = self.deceleration_periods[4][1] if len(self.deceleration_periods) >= 5 else self.carla_data['timestamp'].max()

        steering_data = self.carla_data[(self.carla_data['timestamp'] >= mode_switched_time) & (self.carla_data['timestamp'] <= fifth_end_time)]
        steering_angle_std = steering_data['Steering_Angle'].std() if not steering_data.empty else 0
...
index_csv_path = 'D:\\gitee\\EEG_Neurofeedback\\data_analysis\\results\\20240606\\20240606_trials_index.csv'
for i in range(1, 11):
