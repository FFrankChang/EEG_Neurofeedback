...
for i  in range(10,70,10):
    ...
    # Filter data where Condition is 'feedback's
    feedback_data = data[data['Day'] == 'D02']

    # Group data by 'Day'
    grouped = feedback_data.groupby('Condition')
    ...
    for index, column in enumerate(columns_to_plot):
        ...
        for box, label in zip(bplot['boxes'], grouped.groups.keys()):
            box.set(color='blue' if label == 'feedback' else 'black', linewidth=2)  # Change linewidth if needed
